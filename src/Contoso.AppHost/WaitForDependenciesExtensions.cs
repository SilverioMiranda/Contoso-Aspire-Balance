...
namespace Contoso.AppHost
{
                    ...
                    await foreach (var resourceEvent in resourceNotificationService.WatchAsync(stoppingToken))
                    {
                        if (waitingResources.TryGetValue(resourceEvent.Resource, out var pendingAnnotations))
                        {
                            foreach (var (waitOn, tcs) in pendingAnnotations)
                            {
                                ...
                                {
                                    while(resourceEvent.Snapshot.State is null)
                                    {
    ...
    public static class KafkaHealhCheckExtensions
    {
        /// <summary>
        /// Adds a health check to the Kafka resource.
        /// </summary>
        public static IResourceBuilder<KafkaServerResource> WithHealthCheck(this IResourceBuilder<KafkaServerResource> builder)
        {
            return builder.WithAnnotation(HealthCheckAnnotation.Create(cs => new KafkaHealthCheck(new KafkaHealthCheckOptions
            {
                Topic = "transactions",
                Configuration = new Confluent.Kafka.ProducerConfig { BootstrapServers = cs, 
